...
def getSubCategories(cheats, category):
	subCategories = list(
		set([cheat.subCategory for cheat in cheats if cheat.category == category])
	)
	subCategories.sort()
	...
	cheatsNames = list(
		set(
			[
				cheat.name
				for cheat in cheats
				if cheat.category == category and cheat.subCategory == subCategory
			]
		)
	)
	cheatsNames.sort()
...
def printFormatedCheat(cheat):
	...
	appendToTempFile(
		f"[{green}+{reset}] Sub Category: {underline + yellow + cheat.subCategory + reset}"
	)
...
def printIndice(cheatList):
	...
	for category in categories:
		log.success(f"{big + green + category.upper() + reset}")
...
def findAndGetCorrectCheatNames(cheatList, names):
	...
	cheatNames = [*cheatDict]
	listOfWordsForSearch = names.strip().split()
	finalCheatList = []

	for cheatName in cheatNames:
		if all(word.lower() in cheatName.lower() for word in listOfWordsForSearch):
